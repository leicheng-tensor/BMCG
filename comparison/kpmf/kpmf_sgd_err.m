function [U, V, perct_recovery,time] = kpmf_sgd_err(R, mask, D, K_u_inv, K_v_inv, sigma_r, eta, M0, idx_unknown)
%
% Author: Hanhuai Shan
%
...
for epoch=1:maxepoch
    ...
    perct_recovery(epoch) = get_perct(M0, U*V', idx_unknown);
    disp(['epoch' ,int2str(epoch),', perct_recovery=',num2str(perct_recovery(epoch))]);
    
    if (epoch > minepoch  && (perct_recovery(epoch) - perct_recovery(epoch-1))/perct_recovery(epoch-1)<epsilon)
        break;
    end
