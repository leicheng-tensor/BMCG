function probe_rat_all = bpmf(M,k)
% restart = 1;
% 
% if restart==1 
%     restart=0; 
...
%   makematrix % construct num_p*num_m rating matrix from train_vec(our input)
% 
count = M;
%   load pmf_weight % here let w1_M1 w1_P1 be initialization (use SVD instead)
%   err_test = cell(maxepoch,1);
% r = num_feat;
[a,b,c] = svds(M,k);
w1_P1_sample = a*(b.^(0.5));
w1_M1_sample = (b.^(0.5))*c'; 
w1_M1_sample = w1_M1_sample';
%   w1_P1_sample = w1_P1;
%   w1_M1_sample = w1_M1;
% clear w1_P1 w1_M1;
  ...
  probe_rat_all = w1_P1_sample*w1_M1_sample';
  count=count';
...
probe_rat = w1_P1_sample*w1_M1_sample'+ mean_rating;
...
% bpmf_pred = w1_P1_sample*w1_M1_sample'+ mean_rating;